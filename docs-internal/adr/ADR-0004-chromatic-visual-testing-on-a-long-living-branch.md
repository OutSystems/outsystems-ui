...
2. **Storybook drives the _compiled_ library.** Stories load `dist/*.OutSystemsUI.{js,css}` as classic `<script>`/`<link>` tags and call the public pattern APIs; no story imports anything from `src/`. Consequently **nothing under `src/` exists in Storybook's module graph**, and `dist/` is gitignored.
...
- **Option 3: keep `onlyChanged` and declare an `externals` list covering everything the module graph cannot see** — Pros: correct _and_ keeps savings where they are real. Cons: one more piece of config that must not be "cleaned up" by a future reader.
...
- `externals: [src/**, gulp/**, classic-theme/**]` accompanies `onlyChanged`. A change to any of those paths disables TurboSnap for that build and forces a full re-snapshot. `package.json`/lockfile and `.storybook/**` already force one automatically.
