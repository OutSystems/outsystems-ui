...
| `$token-font-size-200`                                                                                                                              | `0.625rem` (10px)                          | Badge small font size; Icon Badge `.badge` font size; Tag small font size; User Avatar small font size; Bottom Bar Item label font size (hardcoded `0.625rem` / `10px` for now — `$token-scale-250` exists for spacing but not font-size) |
...
| Scale token for `14px`                                                                                                                              | `14px`                                     | Card bottom padding; Animated Label `top: 14px` phone responsive label lift (gap between `scale-300` 12px and `scale-400` 16px)                                                                                                          |
...
| `$token-scale-175`                                                                                                                                  | `7px`                                      | Animated Label `bottom: 7px` (phone responsive offset — between `scale-150` 6px and `scale-200` 8px)                                                                                                                                     |
| Scale token for `30px`                                                                                                                              | `30px`                                     | Carousel `--osui-carousel-pagination-margin` (between `scale-700` 28px and `scale-800` 32px)                                                                                                                                             |
...
| `--osui-bg-neutral-subtlest: #f5f5f5`                          | `src/scss/01-foundations/_root.scss`                                     | **Removed** — declared as an `--osui-*` future-token candidate but never consumed; re-declare alongside the component work that first needs it (never use `--token-*` prefix for hand-declared vars)                                                       |
