...
namespace OutSystems.OSUI.Utils {
	...
	export function ListItemAnimate(
		...
		const listAnimateItems = function () {
			setTimeout(function () {
				const listElement = OSFramework.OSUI.Helper.Dom.GetElementById(ListId).childNodes[1] as HTMLElement;
	...
	export function ShowPassword(): void {
		const inputPassword = OSFramework.OSUI.Helper.Dom.ClassSelector(document, 'login-password') as HTMLInputElement;
