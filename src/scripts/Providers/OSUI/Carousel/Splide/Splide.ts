...
namespace Providers.OSUI.Carousel.Splide {
		...
		implements OSFramework.OSUI.Patterns.Carousel.ICarousel
	{
		...
		private _setListRoles(): void {
			// Remove role="tabpanel" from slides that are themselves, or contain, img/ul/ol/li — elements
			// for which tabpanel ownership is invalid or creates conflicting semantics
			this.selfElement
				...
				.forEach((slide) => {
					...
					if (
						...
						(['UL', 'OL', 'LI'].includes(_slideEl.tagName) || _slideEl.querySelector('img, ul, ol, li'))
					) {
						OSFramework.OSUI.Helper.Dom.Attribute.Remove(
							_slideEl,
							OSFramework.OSUI.Constants.A11YAttributes.Role.AttrName
