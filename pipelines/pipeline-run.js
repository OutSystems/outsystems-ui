...
async function executePipeline(azureProject, pipelineId) {
	...
	let runResults = [];
	const promises = buildResults.map(async (buildResult) => {
		runResults.push(...(await testApi.getTestResults(azureProject, buildResult.runId)));
	});
	await Promise.all(promises);
	const failedWithBugs = runResults
