---
description: Port a new-theme SCSS change into the classic (pre-token) theme snapshot under classic-theme/
argument-hint: "[commit-ish | file path | nothing for the working diff]"
---

# Port to the classic theme

Backport the change described by `$ARGUMENTS` from the token-based new theme into the classic pre-migration CSS snapshot in `classic-theme/`.
...
1. **Resolve the change set.**
   ...
   - If the change set contains no SCSS, stop and say so — there is nothing to port. TypeScript changes have no classic-theme counterpart.
...
5. **Locate insertion anchors** in both `classic-theme/O11.OutSystemsUI.css` and `classic-theme/ODC.OutSystemsUI.css` — the compiled new theme's immediately-preceding rule, so the block lands in the same relative position.
...
7. **Log the port** in `classic-theme/README.md` under a `## Hand-applied ports` heading (create it if absent). One entry per port:
   ```
...
8. **Verify** (skill §8) and report:
   - `grep -c -- '--token-' classic-theme/*.css` → must be 0 for both
   - identical selector/var counts across both files
   - `git diff --stat classic-theme/` → same insertion count per file
   - read `git diff classic-theme/` and confirm the formatting is indistinguishable from surrounding content
...
- the token → classic-var mapping applied
