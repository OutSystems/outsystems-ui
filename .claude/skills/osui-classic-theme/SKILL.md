...
# Porting to the classic (old) theme

## 1. What `classic-theme/` actually is

```
classic-theme/O11.OutSystemsUI.css    ~774 KB
classic-theme/ODC.OutSystemsUI.css    ~781 KB
classic-theme/README.md
```

Two **compiled CSS snapshots** of the pre-token-migration codebase — not SCSS, not partials, not a build target. There is no source tree behind them in this repo; the SCSS they came from lives on `dev` at the commit recorded in `classic-theme/README.md`.

Storybook serves them at `/classic-theme/*` (`.storybook/main.ts` `staticDirs`) and the **Theme** toolbar toggle (`.storybook/preview.ts`) swaps between `/osui/*` (new token theme) and `/classic-theme/*` (old theme) so reviewers can eyeball old vs new.
...
- Hand-applied edits are **destroyed by a snapshot refresh** (rebuild `dev`, copy `dist/*.OutSystemsUI.css` over these). So every port must be logged in `classic-theme/README.md` under **Hand-applied ports** — that log is the only record that survives review and the only way a future refresh can be replayed.
...
```bash
grep -c -- '--token-' classic-theme/*.css      # must stay 0 for both
```

## 3. Token → classic-var mapping
...
The classic-theme snapshot already contains ~40 `--osui-*` custom properties (`--osui-balloon-shape`, `--osui-dropdown-min-width`, …) — the pattern-scoped-variable convention predates the token migration. So **keep the component CSS API structure** when porting; only the *defaults* get de-tokenised:
...
4. **Find the insertion anchor** — the compiled new theme's immediately-preceding rule, located in the classic-theme file so the ported block lands in the same relative position:
   ```bash
   grep -n 'layout.layout-side.aside-overlay .main' classic-theme/*.css
   ```
...
5. **Patch both files** with one script, asserting anchor uniqueness and that the new selector is not already present:
   ```python
   for p in ('classic-theme/O11.OutSystemsUI.css', 'classic-theme/ODC.OutSystemsUI.css'):
...
6. **Log it** in `classic-theme/README.md` under **Hand-applied ports**: date, source commit/ticket, selectors added, and every approximation you had to make.
7. **Verify** (§8).
...
```bash
grep -c -- '--token-' classic-theme/*.css                 # 0, both files
grep -c '<new-selector>' classic-theme/*.css              # identical count, both files
grep -o -- '--osui-<component>-[a-z-]*' classic-theme/*.css | sort | uniq -c   # same vars, same counts
git diff --stat classic-theme/                            # identical insertion count per file
git diff classic-theme/O11.OutSystemsUI.css               # read it; formatting must be indistinguishable
```

Then, for anything visual: `npm run storybook`, flip the **Theme** toolbar to *Classic theme*, and confirm the ported rule takes effect. Also grep that every selector the ported block targets actually exists in the old theme — the old markup contract may differ:

```bash
grep -n 'app-menu-content\|app-login-info\|main-content' classic-theme/ODC.OutSystemsUI.css | head
```
...
- Don't reformat, minify, or run Prettier over `classic-theme/*.css`. They are byte-comparable artefacts; a reformat destroys that.
