...
- **Radius** — shape vocabulary `--border-radius-{none,soft,softer,softest,rounded}`. Soft=8px (`$token-border-radius-200`), softer=12px (`-300`), softest=16px (`-400`), rounded=999px (`-full`). The CSS identifier for 0px is **`none`** (not `rectangular`) because TS `ShapeTypes.Sharp = 'none'`. The matching utility class is **`.shape-rectangular`**. Components read `var(--border-radius, <default>)` so `.shape-*` can restyle chrome. Full constraints: `docs-internal/adr/ADR-0010-border-radius-theme-layer-and-shape-utilities.md`. `none`/`soft`/`rounded` are also read by TS `GetBorderRadiusValueFromShapeType`; `softer`/`softest` are CSS-only.
