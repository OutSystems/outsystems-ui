...
- **`.npmrc` is deliberately untracked** (gitignored), so dev's `min-release-age=7` is not in force here — and cannot be. There is no committed lockfile, so CI resolves from the registry on every run, and this branch consumes `outsystems-design-tokens` within days of publication: `min-release-age` fails the build with `ETARGET` for the first 7 days after every tokens release. Landing on dev needs either a committed lockfile or dev's `.npmrc` dropped.
