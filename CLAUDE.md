...
- **`.npmrc` sets `min-release-age=7`.** A package published in the last 7 days cannot be installed; do not remove the setting to work around it. (The file was briefly untracked while Storybook pulled platform widgets from the internal Azure feed — that dependency is gone, so the file is tracked again and carries no credentials.)
